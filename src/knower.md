...
To understand the Knower, we must first understand what is meant by "knowledge." The easiest way to comprehend it is to contrast knowledge with information. While conventional definitions often see "information" as a tool to predict next event (a stock tip) ; and "knowledge" as the concrete universal reality ( speed of light is constant), we will flip these definitions to align them with the probabilistic quantum world. Probablity is a useful tool because, as observers, our predictive capabilities of most physical processes lack certainty. That is exactly where probabilty shines - to handle uncertainty.
...
> No one ever told "the coin" how to behave. It didn't take probability theory lessons :-) Knowledge is embedded into the shape of the coin. A thick circular sheet of metal with two different shapes on each side that fits right on your thumb and first finger. This shape is meant to be flipped because there is no universe where you could buy a cup of coffee with a coin :-) 
>
> In a way knowledge is something built into every system. It is the basis for weather patterns to emerge, stock markets to function, traffiic in busy down town to the flock of birds - everything has knowledge prebuilt. The gradual unveiling of knowledge (for an observer) is thus akin to "decrypting" a puzzle.

Knowledge means you understand the system inside out, not outside in like an external observer. While a well "informed" may cite evrnt(s), knowledge may hypothesize. It can predict or lay down prophecies. Knower is an insider. It is so deeply entrenched in the system that it is invisible even to ardent observers.
