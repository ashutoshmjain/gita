...
Conventional definitions are fudgy and overlapping.  "Information" is seen as trivial inputs about our environment say "how is the weather tonight?", or "the stock market moved up or down!". And "knowledge" as something grounded in logic or science - for example "speed of light is a constant". A better (and maybe poetical) distinction is "information" is the noise whereas "knowledge" is the signal (hidden in the noise). A practical assumption is that the noise is a small number of possibilities whereas knowledge is a unique signal. The truth is "noise" is infinite (many many orders of magnitude)  and knowledge is just one singular key that unlocks the wisdom. Take for example the multiplication of two super large prime numbers - there are infinitely large numbers of multiplication attempts that will invariably fail for a unique successful attempt. The notion of work is all about finding that unique piece of knowledge encrypted into endless noise.
