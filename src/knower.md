...
Looking at few samples of information, it is hard to ascertain if we are getting closer to the knowledge, or drifting away. Even if we had large set of readings, it is still hard to discern if the path itself was right! You need data at a universal scale, and  a sophisticated approach to parse continuous incoming information streams into right and wrong. There needs to be a way to discriminate wrong in favor of the right path. A methood that allows us ignore all the unwarranted information. This ability must work efficiently whenever the path splits into two - ie every bianry decision - like twenty "yes or no" questions game. This method, in modern AI parlance, is called Value Function and in Vedic science it is called - Viveka. The modus operendi of Viveka is Karmyoga, and in AI parlance, it can only be achieved through actions with the physical world - becauae physical world is a continuous feedback mechanism - it allows the tests in realtime. It determines what is wrong because every action has a consequence - it is ultimate arbeiter of truth. AI can't really develop a value function unless it is acting autonomously outside the domain of thought and language. Which also means it needs absolute decentralization.
