...
`Śakti Muni` added a new rinkle to his father's approach to `Dharma`. He argued - a righteous person, must stick to one path. If someone kept changing her course, how they were any different from flexible animals! And how would they accomplish excellence? `Śakti Muni` coined a new term - `Karm Yoga` - full commitment to one's chosen path. `Karm Yoga` as an idea, was different from the `Nyaan` and `Naad Yoga`. The purpose here was to indulge in actions. The knowledge gained due to repetitive actions was a mere by-product - quite the opposite of the approach of `Nyaan Yoga` wherein the goal was to accomplish with the least effort - find the most efficient way to agriculture, transport or metal works for example. `Śakti's` method didn't seek ascetic behaviour as was expected from `Naad Yogies`. In that sense, it appeared easier to practice - a susutainable path for the masses. All it needed was to perfect one specific skill and keep on doing it - like a machine. In a way, he was the first to underscore specialization. 

<div style="padding-left:40px;font-size:0.9em;"> 

Ascertaining a righteous path is, however, easier said than done. Every time we focus on one task - the task splits into multiple skills - like the branches of a tree. The definitions get nuanced as soon as we peel the first layer. Every human views the reality differently owing to their `Sanskaras` - the core orientations. Even the meaning of words are perceived differently based on the narrative they subscribe to. And above all, humans have a tendency to justify their actions as righteous, because they assume their situation being unique to them - sometimes unaware that such claims were rooted deep into desires. 
