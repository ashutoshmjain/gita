...
Scribing `Mahabharata` was one such venture. In terms of significance, it was probably as big a shift as the computing revolution of late last century. For example, the idea of transcription [^transcription] was to `Mahabharata`, the way coding was to an operating system [^c].  Just as computers disrupted industries through digital flow of information -  and are now disrupting the very foundation of modern trade - the value exchange ; the written word challenged then existing social, fiscal and political fabric. The ground work for that  tectonic shift started much earlier. 
...
`Bramh` is the entirety of information. In that sense `Bramh` also includes the connections we form with the fictitious characters of a narrative. The only difference is the later (connections) are persistent. For example you may forget the names of your co-workers but you would never forget the heroes of your early comic books - particularly if some of those vintage comics were stored somewhere in the attic!  No other species has this luxury to their disposal - books are the batteries of information! A series of books are the blocks of information rigorously mined by the writers that build progressively on the previous. Hence they are deemed as stored persistent chain of information. 
...
The problem, however, was the "recital" must be synchronous! And it was hard to keep it distortion free. No matter how hard a reciter tried, sooner or later, his own point of view would infect the content. More importantly, it was hard to build a chain of information with recital. It appears `Dwaipayana` edited the `Vedas` vocally. `Rig Veda` - the oldest scripture known to mankind is around ten thousand verses long. The realization must have been that it was the limit of not only vocal editing but also vocal learning. There was hardly anyone , except `Dwaipayana` who had all the four `Vedas` stored in memory for recital. In essence four `Vedas` put together are the limit of non-fiction. A bigger chain of information could only be stored in written format and it must be a narrative.


## 0.3
---

Not only did `Dwaipayana` edit four Vedas, he vocally organized eighteen `Puranas`.  There after, as the we reached limits of vocal compilations, he scribed the master piece `Jaya` (victory)- a historical narrative of the early age. History interwoven into thousands of stories meshed up like a knowledge graph. `Jaya` was later, over next six hundred years, expanded into the mega-epic  [Mahabharata](https://mahabharata.shutri.com). With around 1.8 million words, it is the longest written poetic narrative ever, in any language, old or new. To put it in perspective, it is about ten times the size of Iliad and Odyssey combined. More than the size, the poetry is such that it inspired `Kalidasa` (the Shakespeare of Sanskrit) to write the epitome of poetry -  `Abhijnana Shakuntalam` is a rewrite of the plot first conceived by `Dwaipayana`.
