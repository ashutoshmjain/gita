...
While the bulk of Sāṃkhya and Advaita are overlapping, the core difference is in the boundary conditions—origin and dissolution. The Sāṃkhya school of thought considers a perpetual duality of Prakriti and Purusha—the potential and the observer. Advaita considers a definite beginning (and end) from a singularity (seed), albeit in a cycle of demise and regeneration.
...
For an external observer, knowledge unfolds with every new trial. For example, if it was your first time flipping a coin, you would soon realize that heads and tails are the only pair of options and they happen randomly. But there is an order in that randomness—if you flipped enough times, the occurrences of heads and tails are more or less the same. The amazing thing is that this knowledge is inherent to the "coin flipping experiment." No one ever told "the coin" how to behave. The unfolding of knowledge (for an observer) is thus akin to "decrypting" a puzzle. It is as if knowledge was pre-coded into the randomness of possibilities, but there is no way for an external observer to understand how a (complex) system would behave other than by observing it diligently—again and again. 

> Observation is a prerequisite for understanding. The good news is that even though there is no pattern, a solution always emerges amidst the chaos. There are rules at play that ordinary conscious observers such as us humans are unaware of, to begin with, but we have the ability to decrypt them given enough repeated trials.
