...
Conventional definitions are fudgy and overlapping.  "Information" is seen as trivial inputs, for example : "It is 27 degrees this afternoon.", or "the S&P crossed 6000!". And "knowledge" as something profound - grounded in scripture or science - for example "speed of light is a constant" or "love thy neighbor". A better, and maybe poetic, distinction is:  "information" is "noise" whereas "knowledge" is the "signal". 

Looking at few samples of information -it is extremely hard to ascertain if we are getting closer to the knowledge, or drifting away. Even if we had large set of readings, it is still hard to discern if the path itself was right! You need data at a universal scale, and  a sophisticated approach to synthesize continuous incoming information streams. There needs to be a way to discriminate a wrong path in favor of the right one. This allows us to ignore all the unwarranted information associated with the wrong path. This ability must work efficiently whenever the path splits into two - ie every bianry decision - like twenty "yes or no" questions game. This method, in modern AI parlance, is called Value Function and in Vedic science it is called - Viveka. The modus operendi of Viveka is Karmyoga, and in AI parlance, it can only be achieved through interatctions with the physical world - becauae physical world is a continuous feedback mechanism - it allows the test. It determines what is wrong because every action has a consequence - it is ultimate arbeiter of truth. AI can't really develop a value function unless it is acting autonomously in the real world. The immediate feedback loop, at every step, is what AI needs to be able to develop a Value Function, and that is what leads to decentralization.

Collecting noise with five-sensory localized receptors (our senses), and  adjudicating with our predictive faculties (brain and mind), we do an excellent job to ensure survival (increase the window for data collection). But we always lack certainty because value judgement is inherently a quest in the unknown - in the future.  That makes Probability Theory a universal tool. It is not an attempt to brute force through massive data sets - it is a totally different approach to synthsize data. It is a method for abstracting knowledge from noisy information. Something like taking out butter off the curds. From physics standpoint, it is reducton of entropy.
