...
Conventional definitions are fudgy and overlapping.  "Information" is seen as trivial inputs about our environment say "It is going to be 27 degrees this afternoon.", or "the S&P was at 6000 this morning!". And "knowledge" as something grounded in logic or science - for example "speed of light is a constant". A better (and maybe poetical) distinction is:  "information" is the noise whereas "knowledge" is the signal (hidden in the noise). A naive assumption then is:  more information helps to find the knowledge - it can be quite the opposite for it is extremely hard to ascertain if we are getting closer to the knowledge or drifting away by looking at few samples of information. Even if we have large set of readings, it is hard to say if the direction (or gradient) is right - you need significantly large data set - like the entire internet. Equally naive assumption is that the noise (information) is a small number of possibilities and knowledge is a unique signal. The truth is "noise" is infinite (many many orders of magnitude)  and signal can have many peaks on the curve but they are unique in one dimension. Take for example the factorization of of large prime products - there are infinitely large numbers of factorization attempts that will fail for a unique successful attempt but knowledge of any of the primes is a valid solution. The notion of work is all about finding that unique piece of knowledge hidden into endless noise and encrypted in many dimensions.

As five-sensory localized receptors (our senses) of noise, our predictive faculties (mind) do an excellent job to ensure survival, but they always lack certainty - that makes Probability Theory a universal tool. It is not an equation to represent the physiacal reality - it is to peal of confusion of our own noise. It is a method of abstracting knowledge from noisy information. For this essay, we will refine these definitions (Information and knowledge)  to align them with the ideas of probability theory.
