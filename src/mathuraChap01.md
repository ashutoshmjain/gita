...
- **Potency of parity** - Ability to see one's own reflection in `Bramh` is `Siva`. By definition, both `Bramh` and `Vishnu` are infinite. In such vast equilibrium it is impossible to perceive our own reflection. `Siva` thus is the potency to choose most relevant pieces of information. It limits the infinite `Bramh` to a finite set to allow consciousness perceive itself. `Bramh` is our true reflection, but it is to too big to perceive - thus `Siva` is the intelligence that allows us to comprehend truth with exceedingly close approximations. In other words , `Siva` decays the unnecessary - it removes the fluff to bring in clarity. A reflection from infinite dimensions to a self-contained image in comprehensible finite. The notion of self-contained becomes clear at the subatomic levels. For a quark the universe is its antiquark because that is the self-contained reflection -> Separate a quark from its antiquark, and a string forms between them that pulls them back together.  —Kevin Hartnett, Quanta Magazine, 18 Apr. 2023. 
