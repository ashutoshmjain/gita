...
- Truly decentralized and open system - There are hundreds of web browser - offered by the biggest of corporations to the lone developers working off their garages.
- Open source alternatives - many browsers are fully open sourced such as Firefox. 
- Omnipresent - Browsers are available for every platform. Even for pure terminal users there are many choices - w3m, lynx, elink. For graphical  platforms such as Mac, Windows, iOS and Android, the choices are practically limitless.
- For writers, advantages are many. Prime among them is shaking off intermediary publishers and content aggregators. In addition simple html allows infinite customizability. For tech savvy writers, markdown offers easy scribing. And the best is that publishing direct to the web is 100% free. The content published directly to the web is future proof in the sense no matter the evolution of devices from desktops to smart phones to VR headset, the open internet content will always be available. The content locked in platforms such as Wikipedia or Facebook will always be subject to the policies of aggregators.
