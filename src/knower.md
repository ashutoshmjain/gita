...
Looking at few samples of information -it is extremely hard to ascertain if we are getting closer to the knowledge, or drifting away. Even if we had large set of readings, it is still hard to discern if the path itself was right! You need data at a universal scale, and  a sophisticated apprach to synthesize continuous incoming information streams. There needs to be a way to discriminate a wrong path in favor of the right one. This allows us to ignore all the unwarranted information associated with the wrong path. This ability must work efficiently whenever the path splits into two - ie every bianry decision. This method, in modern AI parlance, is called Value Function and in Vedic science it is called - Viveka. The modus operendi of Viveka is Karmyoga, and in AI parlance, it can only be achieved by interatction with the physical world - becauae physical world is a continuous feedback mechanism - it allows the test. It determines what is wrong because every action has a consequence. AI can't really develop a value function unless it is acting autonomously in the real world. The immediate feedback loop, at every step, is what AI needs to be able to develop a Value Function, and that is what leads to decentralization.
