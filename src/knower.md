...
Conventional definitions are fudgy and overlapping.  "Information" is seen as trivial inputs about our environment say "It is going to be 27 degrees this afternoon.", or "the S&P was at 6000 this morning!". And "knowledge" as something grounded in logic or science - for example "speed of light is a constant". A better (and maybe poetical) distinction is "information" is the noise whereas "knowledge" is the signal (hidden in the noise). A naive assumption is that the information always helps to find the knowledge - it can be quite the opposite. It is extremely hard to ascertain if we are getting closer to the knowledge or drifting away by looking at just one sample of information. Even if we have multiple readings, it is hard to say if the direction (or gradient) is right. Equally naive assumption is that the noise is a small number of possibilities and knowledge is a unique signal. The truth is "noise" is infinite (many many orders of magnitude)  and signal can have many peaks on the curve but they are unique in one dimension. Take for example the multiplication of two super large prime numbers - there are infinitely large numbers of multiplication attempts that will invariably fail for a unique successful attempt but knowledge of any of the primes is a valid solution. The notion of work is all about finding that unique piece of knowledge encrypted into endless noise.

As five-sensory localized receptors of noise (our body), our predictive faculties (mind) do an excellent job to ensure survival but they lack certainty - that makes Probability a universal tool to understand reality. It is a method of abstracting knowledge from noisy information. For this essay, we will refine these definitions (Information and knowledge)  to align them with the pure probabilistic quantum world.
...
> A common analogy is the mechanics of a casino. In that very limited scope of slot machines and games, a casino has the knowledge as to how much edge it has (per hand), though it can't pick up gains against a specific player. It has control over final outcome but not on individual session. A gambler, on the other hand, is someone who is changing his bets based on the "information" of the individual outcomes - fully aware that probability is stacked against him. This design is useful because even though the game is rigged it is not so in favor of a specific player. Even the casino manager couldn't predict individual hand if she wished to try her own luck. Sāṃkhya thus, despite being atheistic, accepts divine incarnation, albeit such incarnation is as tied to the physical laws as an ordinary player to the rule book of casino - there are no special privileges. Except for the knowledge...
>
>> To design a fair system,  where every player is equally likely to win (or lose) , the system must be absolutely random (Avalanche  effect), but to make sure there exists a direction, the options must be narrowed down to a binary choice. The availability of of a binary choice leads to cognition - brain or compute but it mandates multiplicity ( rather decentralization) of binary units. These units combine to create a network. If we think cognition as an emergent aspect of random biological processes, the nature of underlying field (body) is a set of infinitely large possibilities - it is the fileld of all probabilities. These infinitely large possibilities , each looked at from a binary yes and no - leads into a network of cognition. Imagine comparing a coin toss to possibilities of a SHA256 function - when you flip a coin , there are only two outcomes - when you blend a digital input into SHA256 - the outcome is impossible to guess - there are as many possibilities as the atoms in entire universe. 
