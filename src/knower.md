...
Conventional definitions are fudgy and overlapping.  "Information" is seen as trivial inputs about our environment, for example : "It is 27 degrees this afternoon.", or "the S&P 500 was at 6031 this morning!". And "knowledge" as something grounded in logic or science - for example "speed of light is a constant". A better (and maybe poetical) distinction is:  "information" is seen as "noise whereas "knowledge" is the "signal" (hidden in the noise). Looking at few samples of information -it is extremely hard to ascertain if we are getting closer to the knowledge or drifting away . Even if we had large set of readings, it is still hard to say conclusively if the path itself was right! You need data at a universal scale, and  a sophisticated apprach to synthesize massive data sets. There needs to be a way to discriminate against a wrong path (and entire associated information) - to choose the right one. This ability must work efficiently whenever the path splits into two - ie every bianry decision. This method, in modern AI parlance, is called Value Function.

Collecting noise from five-sensory localized receptors (our senses), our predictive faculties (brain and mind) do an excellent job to ensure survival (increase the window for data collection), but they always lack certainty - that makes Probability Theory a universal tool. It is not an attempt to brute force through massive data sets - it is a totally different approach to synthsize data. It is a method for abstracting knowledge from noisy information. Thus, we will refine core definitions (Information and knowledge)  to align them with the ideas of probability theory - a subtle change but a profound one!
