...
> `Karma` is the action that converts information into knowledge. `ViKarma` are the actions that convert knowledge into information. For example when we practice to improve our game, it is `Karma` , when we play a match to put our skills on display, it is called  `ViKarma`. Both are equally important. A set of predefined `Karmas` repeated at a predefined frequency is called `Yazna`. `Yazna` leads to success. For example when you study everyday for eight long years you become a doctor. Same is true for an Engineer, a Scientist, an Economist and thousands of other specializations . The daily ritual of study and lab in this case is `Yazna`. `Yazna` becomes effective when performed alone and without desires.
