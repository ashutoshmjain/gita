...
Intelligence and knowledge are at dance with each other. `Yajna` is iterative process to weave the mesh such that the knowledge is caught into it and is forced to manifest. Each iteration of `Karma` in `Yajna` is a progressive refinement of the mesh , of the structure to enable the capture. The manifestation happens instantaneously as soon as the mesh is just right. 

Since we are using the example of the mesh - think about the net to capture the fish. It must have taken thousands of iterations to perfect the net. And the ritual of casting the net in the sea. The moment net is cast , under right conditions, the fish automatically gets captured. You may say capture of fish is NOT instantaneous. We will talk about it in the section on {time}.

Take another example of say Apple App store. As the intelligence perfected the mechanism of consumption and delivery of mobile apps, millions of apps showed up in the app store. Unlike fish , we don't know where from these apps came. Most of them didn't exist before. As the network got right , the apps just manifested out of nowhere. Did the fish also got manifested in the net ? Or they travelled all the way to the net to meet their demise ? A question we will ponder over later .. 
...
It happens all the time. You were building a great culture at your organization and suddenly you were fired. Financial crisis happens. Pandemics happen. These things are totally beyond our control. Accidents , calamities don't even signal upfront. What you do. You lost your `Nishkaam Yajna`. What a waste. You are resigned to fate. Nothing works. 

As we talked above; the acts we do are always part of a larger ecosystem. There are trillions of things going at the same time. One such thing can derail your plans. Should we call it fate ? Or we call it `Maya` - the matrix. Nothing is beyond `Maya` - even the free open waters before you were caught into this net. Matrix is everywhere. 
