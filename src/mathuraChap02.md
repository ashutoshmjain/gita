...
> The knowledge of written symbols starts long before a child actually holds a pen. We create an association of a specific sound with a selected text - "A" is a sound as well as a written symbol. They are reflection of each other. A sound with it's written reflection is the fundamental unit of extrenal store of knowledge. 

Despite tremendous advances in our learning systems, this fundamental association stayed unchanged. Associations may be different in different languages but there is always a sound and a symbol of text. It is a universal tenet of education. This was the first innovation in the education system. The most crucial aspect of education, as per `Vasishtha`, was a child's first exposure to the spoken words. The `Sanskaras` - the core orientations, are formed at child's first learning steps - he thought. That was one reason, mother was considered the first teacher. The association of spoken word and the `Sanskara` was considered as old as the human race. Adding a twist of written text in this primordial association of learning was what excited `Bramhrishi` in his rebooted reality.

> In last hundred years, education has seen tremendous growth. The modern education system of schools and online platforms is great but they all deal with grownups. Despite thousands of years, the first and fundamental learning steps are almost unchanged from what `Vasishtha` discovered for `Parasara`. We might use an iPad for showing the picture of Apple but it is still A for Apple - a written symbol and a sound simultaneously associating with a real world picture.

However, what `Vasishtha` invented was only an "innovation", not an original idea. It was inspired from the discovery of "music" by `Siva`. In music, a note was identified with a name. "Sa Re Ga Ma" or "Do Re Me Saa" are nothing but the spoken names for eternal sounds. In essence, `Siva` named the musical notes with spoken symbols , `Vasishtha` named spoken sounds with written symbols. Though `Vasishtha` went a step further - he added the idea of text into `Sanskaras` - a revolutionary change in the kernel of learning system. 

The grandfather used to take the budding child to the river `Yamuna` for a daily bath. He often drew the symbols on the sandy banks of the mighty river with a stick and had `Parāśara` guess the letters with the same object as a clue held in his hand. `Parāśara` often complained when the mighty river wiped off the text during a high tide. `Vaśiṣṭha`, the all aware, keeper of the knowledge, used to smile to himself thinking one day these written symbols would be indestructible and immutable (`Akshra`) like the knowledge itself. It is probably why the written word is called `Akshra` in `Sanskrit`!
