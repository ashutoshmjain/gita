...
To appreciate the infinite expanse, we may describe `Hari` as some sort of omni-present field. His incarnations are mere representation of a deeper underlying substrate that is beyond our senses — a concious field maybe, but more precisely a quantum system, one that exists and baffles scientific thinkers as much as followers of scriptures. 

Our limitation is we can only perceive what our senses allow us to. The good news is, we have an innate ability to allow us understand one deeper layer - we can peal the onion, but only once — the top layer. It happens with hard work but more importantly if we get lucky. It doesn't really matter if the tools to peal the outer layer are tricks of science or math; or they are spiritual in nature; or the business; or even sports. Any path may enable us peal one layer — one layer of our experience. If done right - it opens up a door to the infinite — we become part of the system that spans our universe, or may be many more — we call this system `Bramh`. This is the start of `Karma`. From here, it is a very long journey to reach `Hari` - the source code that runs `Bramha`. When we fully understand the source, then we become the super-conscious rule-keeper - `Siva`. `Siva` gets discretion. For most that is the end. 

Only one reincarnates as `Hari`. Infinite, able, unexpressed yet the reason of awareness. One conscious. Entire `Bramh` and many `Siva` bow to this new incarnation of `Hari`. He is complete. Matrix doesn't bind him. That is the reason he can change the rules of the game - upgrade the matrix to a new consciousVerse - a new version of `Bramh` and `Siva` soon arrive thereafter. Every new consciousVerse is an exponential increase. Thus we call these incarnations inflationary.

After ten upgrades, tenth `Hari` creates a totally new physical realVerse with his eight basic natures (`Ashtdhamool Prakriti`) —  such that consciousness moves into a new quantum system and "time" restarts. Having lost the "Time" the old systems stay there for ever. Nothing is ever destroyed, it is just that it loses relevance. The cycle repeats endlessly. Even `Hari` can't stop the "Spin".

Eighth incarnation if `Hari` is complete because all the aspects of basic nature are expressed in this incarnation. Ninth incarnation is all about extending the life of this physical verse. Tenth incarnation is all about changing the physical to a new reality.  Same evolutionary cycle works from tiniest microbes or sub atomic units, to the gargantuan galaxies and universes. Only one set of universal rules play at all the levels of infinite expanse with in one realVerse.
...
It took almost hundred years to understand this phenomenon at a deeper level. During this period , electricity and  
