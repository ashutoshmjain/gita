...
There are three basic colors that our [eyes](https://www.allaboutvision.com/eye-care/eye-anatomy/color-vision) are designed for - he said - Red, Green and Blue [^RGB]. They mix up to make millions of colors. Our eyes are limited to around ten million shades. Possibilities are endless, but four new colors are rather stable - Orange, Yellow, Indigo and Violet. Together with three basic ones, the seven colors are, as we all know, the colors of a rainbow. They are normally hidden in one "transparent invisible" light but they reveal themselves when elements meet - after the rain when the sun shows up - the suspended droplets, on tiny dust particles, light up the washed air. As if there is a spectral choreography of all that could be observed. The cosmos shows up its true nature - and beautiful it is! Isn't it? For the sake of conversation, we refer to the transparent light as white light, as opposed to absence of light that makes everything dark.

He paused to check if he made sense, and also to make sure he was not overtly poetical.
...
Complete is simple to explain - it means, if you look at the group of all colors as a whole, the locked in knowledge is awareness of all the separations. In other words, it is an algorithm that converts a given color into its corresponding anti-color. Means it doesn't only know Magenta is complimentary to Green , it knows the complementarity of all the possible colors  - replied Angel. Identity is little hard to fathom - he continued. It means that this knowledge is inbuilt into all the colors. It is in every member of the group but it doesn't change the appearance of the any of the constituents. In other words, the independent color can't manifest without imbibing the algorithm - this algorithm is the operation that creates the separation in first place. It also means that when two opposing members mix themselves to annihilation - what remains is the common denominator - the Identity.

This identity - white transparent light in case of our group of seven rainbow colors - has it's own inverse - the utter darkness. In other words, the white light is born of seven underlying separations - Seven knots in `Vaidik` speak , and it separates itself from the darkness.  No matter how complex an identity, it always has it's inverse  - it must separate [^separation] from something else to become observable.
