...
No one could fathom to what avail, but the cause appeared worth dedicating many lives. Thousands of scribes would spend millions of hours just to copy, rewrite and improve the open root narrative just as Unix morphed into BSDs, Solaris, AIX, HP-UX, Minix, Linux and Android and many more including countless variants of each. In later case, they all carried forward what is commonly known as Unix philosophy - somewhat like an embodiment of universal values that bind all of them into one single rendition - for example seemingly irrational idea of "open source" evolved off those values. It defies conventional wisdom - why would you distribute your code free to any random person ?

As described in the very first book of `Mahabharata` (Adi Parva), the `Vaidik` editor-in-chief (Ved-Vyasa), sage teacher of all the races [^races], immortal `Krishna Dwaipayana` invoked `Bramha` - the embodiment of all-pervading information, to accomplish the pursuit. `Mahabharata` was the first abstraction of past activities into written words - an abstraction of historical events into a single narrative - free to everyone to experience history in their respective minds. 

> The idea of abstraction of actions (**thought**) , henceforth, would percolate beyond races, geographies and cultures to define a datum for humans. All future ideologies (religions and faculties of science) must be built on a rock solid treatise - Gita , Bible, Quran, Physica, Principia, Relativity - and hundreds of more.  

Spoken words  and actions were synchronous [^word] -> Thoughts were an abstraction over actions -> When we think, we think about our next set of actions without indulging in the real world. Or we ponder over the past actions. Thoughts manifest into written words. In essence the written words are an abstraction over physical actions - similarly as code is an abstraction over thoughts !

It took us a while to understand and appreciate the idea that code is an abstraction over thoughts. It didn't become clear till we got to artificial intelligence and when we did, suddenly everything started making sense. For example - in realm of artificial intelligence, hallucinations are abstractions over "thoughts".  When AI hallucinates, it essentially has conflicting sets of thoughts that lead to weird answers. It is now quite clear that AI is a derivative of thoughts whereas thoughts were cemented over actions. For AI the path is thus Hallucinate > think > act. It hallucinates a lot if trained with random conflicting data! Reinforced learning is expected to reduce [hallucinations](https://robots.net/news/are-ai-models-doomed-to-always-hallucinate/).

Invocation of the primal force, at neural levels, is an invitation to conflicting ideas to play all their cards. We must cut through the clouds to leap into open space. We must go through the churn before we see the inevitable. As we say about AI - it must hallucinate before it sees the truth !
