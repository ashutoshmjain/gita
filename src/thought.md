...
Randomness and memory allow artificial general intelligence to expand or excavate a narrative - that is what we do ! The question, however, still remains - how do we transmit value across these virtual constructs ?
...
In `Vaidik` science, being one with `Bramh` is the notion of being one with the continuum. One may experience any narrative of their choice at any time - to consume, to learn, to play or to understand. Choice to be in a narrative does come at a certain cost besides the risk that one may get lost in the narrative if the experience is truly immersive. In other words, each narrative that we choose to spend our time in comes with an invoice - we pay this invoice with `Karma`. We accomplish `Karma` through free-will and memory - inside the experience. When we are paid in full, we have choice to exit or stay longer to enjoy this beautiful sensory interface. And we continue to work till we paid in full. All the payments go back to the continuum. `Vaidik` science also adds that we can even change the bodies that we use to experience the chosen narrative - any shape or size that we deem fit for the purpose, just as we choose the avatars in a game.
