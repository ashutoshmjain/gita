...
Despite this path-breaking idea, `Bramhrishi` stayed humble for what he invented was only an "innovation" - not an original idea. It was inspired from the discovery of "music" by `Siva`. In music, a note is identified with a name. "Sa Re Ga Ma" or "Do Re Me Saa" are nothing but the spoken names for seven eternal sounds. In `Hindu` mythology, music and dance (as recognized art forms) were created by supreme potency of discretion - `Siva`. On the same lines, `Vasishtha` named the written symbols. However, `Vasishtha` went a step further - unlike music which was an optional hobby and remains same to date, he added the idea of text into `Sanskaras` - the first learning steps - a revolutionary change in the kernel of learning system. 

The grandfather used to take the budding child to the river `Yamuna` for a daily bath. He often drew the symbols on the sandy banks, with a stick, and had `Parāśara` guess the letters. `Parāśara` often complained when the mighty river wiped off the overnight text during a high tide. `Vaśiṣṭha`, the all aware, keeper of the knowledge, used to smile wondering one day these written symbols would be indestructible (`Akshra`) like the knowledge itself. It is probably why the written word is called `Akshra` (immutable) in `Sanskrit`!

`Vaśiṣṭha` had no clue how scribing the entire knowledge base would fructify for `Devnagari` back then supported only eight [^eight] rudimentary sounds, just enough to send cryptic pigeon messages. He knew more symbols were needed if the intent of scripture was to be meaningfully conveyed but he also knew a mighty ocean started with a single droplet. His goal was to see `Parāśara` as first child natively raised with speech and scribing hand in hand. He probably wanted to understand if it was even possible? And if it were, than what is the effect on child's approach to learning?

He sure knew if he could train one child for the written word, than at some point, everyone would follow his method, instead of a "typical learning path" - focused solely on "speech". And if everyone knew writing, than writing all the `shruties`[^shruti] would be an easy shared effort. Not only an easy shared effort, it will also ensure location decentralization. If scriptures were written at distant places, it would be impossible to destroy them. Instead of transcribing "knowledge" on temple walls, he wanted to enable all humans to transcribe. Empowering people through "speak and scribe" (`Akshar Gyaan`) - he saw -  as an insurance against similar disasters!
