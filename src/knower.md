...
>
>> To design a fair system,  where every player is equally likely to win (or lose) , the system must be random (Avalance effect), but to make sure there exists a direction, the options must be narrowed down to a binary choice. The availability of of a binary choice leads to cognition - brain or compute. if we think cognition as an emergent aspect of random biological processes, the nature of underlying field (body) is a set of infinitely large possibilities - it is the fileld of all probabilities. Imagine comparing a coin toss to possibilities of a SHA256 function - when you flip a coin , there are only two outcomes - when you blend a digital input into SHA256 - the outcome is impossible to guess - there are as many possibilities as the atoms in entire universe. 
