...
because this knowledge is not easily perceived by our externally focused senses or mind. Our senses and mind are designed to ensure survival to lend us a window to explore this reality. Their purpose is to focus on external things thus it appears encrypted `Avykta` to our senses. And the reality around us holds the key to our inner knowledge.  Since everyone has a different set of circumstances , it is not hard to imagine that as an observer we see a very different world. As we said earlier, thus the test is individualistic. BTW it is important to state the meaning of "encrypted". Encrypted doesn't mean "impossible". It only means that it is hard to decrypt unless we devise a better algorithm. Better ways and means to understand the problem. Or we have the public private key combination. Once we have the keys, the entire knowledge base becomes self evident. 
...
2. The knowledge appears encrypted to our externally focused senses and mind but it is knowable with proper rigor and algorithms.
3. We manifest ourselves and the things around us based on the knowledge we decrypt.
4. The collective decrypted knowledge is called `Bramha` - The pan universal knowledge cloud. 
