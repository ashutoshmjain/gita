...
Just like modern science, `Vaidik science` had two distinct flavors - theory and application. The application part is what we discussed above  - the 100 skills - metallurgy, agriculture, medicine etc. The theoretical part concerned itself to the origin and demise of the universe - mostly without proof because the focus was on what was necessary for survival rather than understanding boundary conditions. However, we still needed a logical framework to rest our anxiety - why sun shows up? What is at the other end of sky! What happens when we die and host of other questions that may not add immediate value to daily life but if left unaddressed, may cause unrest. 

The biggest contribution of ten generations of `Vasishthas` was a framework to rest human anxiety. Three words `mantra` - `Om Tat Sàt` covered the questions of origin and rise - how this universe came together - how is it perpetuated, and what it means ?  `Om` signified the origin - the source. `Tat` signified all observable - the creation. And `Sàt` meant truth that the creation bears in itself.
