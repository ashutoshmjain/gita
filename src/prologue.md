...
No one could fathom to what avail, but the cause appeared worth dedicating many lives. Thousands of scribes would spend millions of hours just to copy, rewrite and improve the open root narrative just as Unix morphed into BSDs, Solaris, AIX, HP-UX, Minix, Linux and Android and many more including countless variants of each. In later case, they all carried forward what is commonly known as Unix philosophy - somewhat like an embodiment of universal values that bind all of them into one single rendition - for example the idea of "open source" evolved off those values.

As described in the very first book of `Mahabharata` (Adi Parva), the `Vaidik` editor-in-chief (Ved-Vyasa), sage teacher of all the races [^races], immortal `Krishna Dwaipayana` invoked `Bramha` - the embodiment of all-pervading information, to accomplish the pursuit of `Mahabharata` - an abstraction of historical events into a single narrative. The idea of abstraction of actions (**thought**) , henceforth, would percolate beyond races, geographies and cultures to define a datum for humans. All future ideologies (religions and faculties of science) must be built on a rock solid treatise - Gita , Bible, Quran, Physica, Principia, Relativity - and hundreds of more.  

> Spoken words [^word] and actions were synchronous -> Thoughts were an abstraction over actions -> When we think, we think about our next set of actions without indulging in the real world. Or we think about the past actions where indulgence in real world is impossible. Thoughts manifest into written words.
>
> In realm of artificial intelligence, hallucinations are abstractions over "thought"  AI is a construct in thought whereas thought was cemented over actions. For AI the path is thus Hallucinate > think > act. It hallucinates a lot if trained with random conflicting data! Reinforced learning is expected to reduce [hallucinations](https://robots.net/news/are-ai-models-doomed-to-always-hallucinate/). 
>
> What about natural intelligence? Do we experience conflicting thoughts?
