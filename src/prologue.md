...
- In Vaidik parlance Permission-less and censor resistant stands for "open to all without any controls" and "for benefit of everyone who wanted to subscribe". This ideology was termed `Sanatana` - means one that has no beginning and no end - ever present. The underlying value exchange model of `Sanatana` are `Karma` - work for a specific skill. And the measure of `Karma` is in proof of work - `Vikarma`. `Sanatana` believes in immutable record of `Karma` - a universal immutable ledger.
